...
@Component({
    selector: 'submenuMulti',
    templateUrl: './submenuMulti.component.html'
})

export class SubMenuMultiComponent implements OnInit {
    subdropdown: SubMenuView[] = [];
    ...
    geMenuListList() {
        this.menuservice.getAllSubmenuForDropdown()
            .subscribe(data => {
    ...
    getSubMenuListList() {
        this.menuservice.getListOfSubmenu().subscribe(
    ...
    public submit() {
        this.loading = true;
        this.menuservice.createMultiLevelSubMenu(this.model)
            .subscribe(
                data => {
